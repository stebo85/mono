...
| Per-level LOD brightness/opacity compensation | ✅ | `lodGammaExponent` (colour, default `0.022`) folds into the display-gamma exponent per brick; `lodOpacityScale` (alpha, default `0` — measured worse on dense structure) scales the step-size opacity exponent. Both backends; colour also on 2D slice tiles, opacity is ray-march only |
